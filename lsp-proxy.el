...
;; (defvar-local lsp-proxy--chunk-call-count 0
;;   "Counter for chunk loading calls to detect infinite recursion.")

;; (defvar-local lsp-proxy--last-chunk-time nil
;;   "Timestamp of last chunk loading call.")
    ...
    (when (and file-path
      ...
      (with-current-buffer buffer
        (setq lsp-proxy--large-file-loading-state 'pending
              ...
              ;; lsp-proxy--chunk-call-count 0
              ;; lsp-proxy--last-chunk-time nil
              ))
...
(defun lsp-proxy--load-next-chunk (buffer file-path chunk-index chunk-size total-size loaded-size)
  ...
  ;; Add logging and recursion detection
  ;; (when (buffer-live-p buffer)
  ;;   (with-current-buffer buffer
  ;;     (setq lsp-proxy--chunk-call-count (1+ (or lsp-proxy--chunk-call-count 0)))
  ;;     (let ((current-time (current-time))
  ;;           (time-since-last (when lsp-proxy--last-chunk-time
  ;;                             (float-time (time-subtract (current-time) lsp-proxy--last-chunk-time)))))
  ;;       (setq lsp-proxy--last-chunk-time current-time)

  ;;       ;; Log every call with detailed info
  ;;       (message "[LSP-PROXY-CHUNK] Call #%d: chunk=%d loaded=%d/%d (%.1f%%) state=%s time-since-last=%.3fs"
  ;;                lsp-proxy--chunk-call-count
  ;;                chunk-index
  ;;                loaded-size
  ;;                total-size
  ;;                (* 100.0 (/ (float loaded-size) (float total-size)))
  ;;                lsp-proxy--large-file-loading-state
  ;;                (or time-since-last 0.0))

  ;;       ;; Detect potential infinite recursion
  ;;       (when (and time-since-last (< time-since-last 0.05)) ; Less than 50ms between calls
  ;;         (message "[LSP-PROXY-CHUNK] ⚠️  WARNING: Very fast recursion detected! (%.3fs between calls)" time-since-last))

  ;;       ;; Safety check: prevent infinite recursion
  ;;       (when (> lsp-proxy--chunk-call-count 1000)
  ;;         (message "[LSP-PROXY-CHUNK] 🚨 ERROR: Too many chunk calls (%d), stopping to prevent infinite loop!" lsp-proxy--chunk-call-count)
  ;;         (setq lsp-proxy--large-file-loading-state 'failed)
  ;;         (lsp-proxy-cancel-large-file-loading)
  ;;         (return)))))
  ...
  (when (and (buffer-live-p buffer)
    ...
    (let* ((start-pos loaded-size)
      ...
      (if is-last-chunk
          (progn
            ;; (message "[LSP-PROXY-CHUNK] ✅ Completing large file loading")
            (lsp-proxy--complete-large-file-loading buffer file-path))
        (progn
          ;; (message "[LSP-PROXY-CHUNK] ⏰ Scheduling next chunk #%d in 1s" (1+ chunk-index))
          (run-with-idle-timer
           1 nil
...
(defun lsp-proxy--complete-large-file-loading (buffer file-path)
  ...
  (when (buffer-live-p buffer)
    (with-current-buffer buffer
      ;; (message "[LSP-PROXY-CHUNK] Final stats: total calls=%d state=%s"
      ;;          (or lsp-proxy--chunk-call-count 0)
      ;;          lsp-proxy--large-file-loading-state)
...
(defun lsp-proxy--on-doc-open ()
  ...
  (when buffer-file-name
    (when (not (f-exists? buffer-file-name))
      (save-buffer))
    (let* ((initial-content (if lsp-proxy--is-large-file
    ...
    (lsp-proxy--notify 'textDocument/didOpen
                       (list :textDocument (append (eglot--TextDocumentIdentifier)
                                                   (list
                                                    ...
                                                    :languageId ""
                                                    :version (if lsp-proxy--is-large-file -1 eglot--versioned-identifier)
                                                    :isLargeFile lsp-proxy--is-large-file))))
    ;; send large file content
    (when lsp-proxy--is-large-file
